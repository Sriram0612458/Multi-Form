...
}) =>
{
    ...
    ease-in-out delay-300 ${step == currentIndex
            ...
            {[...Array(totalSteps - 1)].map((_, idx) => (
                <div key={idx} className="flex items-center w-full">
                    ...
                    <div className="flex-grow h-[2px] sm:h-1 relative">
                        ...
                        <div
                            className={`absolute top-0 left-0 h-full w-full bg-blue-500 transition-transform duration-300 ease-in-out origin-left transform ${step > idx + 1 ? "scale-x-100" : "scale-x-0"}`}
                        />
                    </div>
                </div>
            ))}
